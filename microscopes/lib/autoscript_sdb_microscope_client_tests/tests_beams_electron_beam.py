...
class TestsBeamsElectronBeam(unittest.TestCase):
    ...
    def test_switch_scanning_modes(self):
        ...
        # Unblank beam; beam is blanked as a side-effect of scanning mode switching on some platforms
        print("Unblanking beam...")
        microscope.beams.electron_beam.unblank()

        if self.test_helper.is_system_version(6):
            # Grab one frame as another workaround for a bug in XT.
            # On XT 6 systems pipeline is in some weird state after the scan mode switching storm.
            # The pipeline is running and cannot be stopped from XTUI. Setting active detector subsequently fails.
    ...
    def test_beam_deceleration(self):
        ...
        if not self.test_helper.is_beam_deceleration_installed:
            self.skipTest("Beam deceleration not installed, skipping")
        ...
        # Tilt to 0 degrees, BD is not allowed with high stage tilts
        print("Tilting stage to 0 degrees...")
        microscope.specimen.stage.absolute_move(StagePosition(t=0))
        print("Success.")
    ...
    def test_unsupported_beam_deceleration(self):
        ...
        if self.test_helper.is_beam_deceleration_installed:
            self.skipTest("Beam deceleration installed, skipping")
    ...
    def test_angular_correction(self):
        angular_correction = self.microscope.beams.electron_beam.angular_correction

        with self.assertRaises(ApplicationServerException):
            print("Testing unsupported mode...")
            self.microscope.beams.electron_beam.angular_correction.mode = "Unsupported"

        print("Testing angle...")
        self.microscope.beams.electron_beam.angular_correction.mode = AngularCorrectionMode.MANUAL
        angle_limits = angular_correction.angle.limits
        angle_value = (angle_limits.min + angle_limits.max) / 3
        angular_correction.angle.value = angle_value
        self.assertAlmostEqual(angular_correction.angle.value, angle_value)

        with self.assertRaises(ApplicationServerException):
            print("Testing angle while in automatic mode...")
            self.microscope.beams.electron_beam.angular_correction.mode = AngularCorrectionMode.AUTOMATIC
            angle_value = (angle_limits.min + angle_limits.max) / 2
            angular_correction.angle.value = angle_value

        print("Testing specimen pretilt...")
        self.microscope.beams.electron_beam.angular_correction.mode = AngularCorrectionMode.AUTOMATIC
        pretilt_limits = angular_correction.specimen_pretilt.limits
        pretilt_value = (pretilt_limits.min + pretilt_limits.max) / 2
        angular_correction.specimen_pretilt.value = pretilt_value
        self.assertAlmostEqual(angular_correction.specimen_pretilt.value, pretilt_value)

        with self.assertRaises(ApplicationServerException):
            print("Testing specimen pretilt while in manual mode...")
            self.microscope.beams.electron_beam.angular_correction.mode = AngularCorrectionMode.MANUAL
            pretilt_value = (pretilt_limits.min + pretilt_limits.max) / 3
            angular_correction.specimen_pretilt.value = pretilt_value

        print("Testing dynamic focus...")
        self.microscope.beams.electron_beam.angular_correction.dynamic_focus.turn_off()
        self.assertEqual(self.microscope.beams.electron_beam.angular_correction.dynamic_focus.is_on, False)
        self.microscope.beams.electron_beam.angular_correction.dynamic_focus.turn_on()
        self.assertEqual(self.microscope.beams.electron_beam.angular_correction.dynamic_focus.is_on, True)

        print("Testing tilt correction...")
        self.microscope.beams.electron_beam.angular_correction.tilt_correction.turn_off()
        self.assertEqual(self.microscope.beams.electron_beam.angular_correction.tilt_correction.is_on, False)
        self.microscope.beams.electron_beam.angular_correction.tilt_correction.turn_on()
        self.assertEqual(self.microscope.beams.electron_beam.angular_correction.tilt_correction.is_on, True)

        print("Done")

    def test_source_is_broken(self):
        print("Retrieving whether source is broken...")
        try:
            source_is_broken = self.microscope.beams.electron_beam.source.is_broken
        except ApplicationServerException as e:
