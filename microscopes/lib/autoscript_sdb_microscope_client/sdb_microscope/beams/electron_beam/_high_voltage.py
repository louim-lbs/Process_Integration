...
class HighVoltage(object):
    ...
    def __init__(self, application_client):
        ...
        self.__id = "SdbMicroscope.Beams.ElectronBeam.HighVoltage"


    @property
    def is_controllable(self) -> 'bool':
        """
        Retrieves the information about the controllability.
        """
        call_request = CallRequest(object_id=self.__id, method_name="IsControllable_GET")
        call_response = self.__application_client._perform_call(call_request)
        if call_response.result.data_type != DataType.BOOL:
            raise TypeError("Incompatible type: high_voltage.is_controllable was expecting bool, but server returned different object type: " + repr(call_response.result.data_type))
    ...
    def limits(self) -> 'Limits':
        ...
        if call_response.result.data_type != DataTypeDefinition(DataType.STRUCTURE_PRIMARY_ID, secondary_id="Limits"):
            raise TypeError("Incompatible type: high_voltage.limits was expecting Limits, but server returned different object type: " + repr(call_response.result.data_type))
    ...
    def value(self) -> 'float':
        ...
        if call_response.result.data_type != DataType.DOUBLE:
            raise TypeError("Incompatible type: high_voltage.value was expecting float, but server returned different object type: " + repr(call_response.result.data_type))
