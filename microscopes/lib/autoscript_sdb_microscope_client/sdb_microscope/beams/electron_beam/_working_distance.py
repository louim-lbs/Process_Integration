...
class WorkingDistance(object):
    ...
    def __init__(self, application_client):
        ...
        self.__id = "SdbMicroscope.Beams.ElectronBeam.WorkingDistance"


    def set_value_no_degauss(self, value: 'float'):
        """
        Sets the value of the working distance with the automated Immersion lens degauss procedure disabled.
        
        :param value: New value of the working distance in meters.
        """
        call_request = CallRequest(object_id=self.__id, method_name="SetValueNoDegauss", signature= [DataType.DOUBLE], parameters=[value]) 
    ...
    def limits(self) -> 'Limits':
        ...
        if call_response.result.data_type != DataTypeDefinition(DataType.STRUCTURE_PRIMARY_ID, secondary_id="Limits"):
            raise TypeError("Incompatible type: working_distance.limits was expecting Limits, but server returned different object type: " + repr(call_response.result.data_type))
    ...
    def value(self) -> 'float':
        ...
        if call_response.result.data_type != DataType.DOUBLE:
            raise TypeError("Incompatible type: working_distance.value was expecting float, but server returned different object type: " + repr(call_response.result.data_type))
