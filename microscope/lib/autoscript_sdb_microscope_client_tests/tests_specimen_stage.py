from autoscript_core.common import ApiException, ApplicationServerException
...
class TestsSpecimenStage(unittest.TestCase):
    ...
    def test_move_stage_with_zy_link(self):
        microscope = self.microscope
        stage = microscope.specimen.stage
        pi = math.pi

        if self.test_helper.is_offline:
            self.skipTest("Link Z-Y positioning option is not simulated in offline mode, skipping")

        if not stage.is_installed:
            self.skipTest("Bulk stage is not installed, skipping")

        if not self.test_helper.is_link_zy_supported:
            with self.assertRaisesRegex(ApplicationServerException, "not supported"):
                settings = MoveSettings(link_z_y=True)
                stage.absolute_move(StagePosition(0), settings)

            self.skipTest(f"Link Z-Y positioning option is not supported on XT {self.test_helper.get_system_major_version()}, skipping")

        # Assuming the stage is linked and at eucentric from previous test_move_stage() run
    ...
    def test_move_stage_with_compucentric_tilt(self):
        microscope = self.microscope
        stage = microscope.specimen.stage
        pi = math.pi

        if self.test_helper.is_offline:
            self.skipTest("Compucentric tilt positioning option is not simulated in offline mode, skipping")

        if not stage.is_installed:
            self.skipTest("Bulk stage is not installed, skipping")

        if not self.test_helper.is_compucentric_tilt_supported:
            with self.assertRaisesRegex(ApplicationServerException, "not supported"):
                settings = MoveSettings(tilt_compucentric=True)
                stage.absolute_move(StagePosition(0), settings)

            self.skipTest(f"Compucentric tilt positioning option is not supported on XT {self.test_helper.get_system_major_version()}, skipping")
        ...
        print("Tilting without compucentric tilt...")
        stage.absolute_move(StagePosition(t=15 * pi / 180), MoveSettings(tilt_compucentric=False))
        y_when_tilted_without_link = stage.current_position.y
        print("Y when tilted is", y_when_tilted_without_link)
        self.assertAlmostEqual(y_at_zero_tilt, y_when_tilted_without_link, 5)
        print("Success.")
        ...
        print("Tilting with compucentric tilt...")
        stage.absolute_move(StagePosition(t=15 * pi / 180), MoveSettings(tilt_compucentric=True))
        y_when_tilted_with_link = stage.current_position.y
        print("Y when tilted is", y_when_tilted_with_link)
        self.assertNotAlmostEqual(y_at_zero_tilt, y_when_tilted_with_link, 5)
        print("Success.")
    ...
    def test_move_stage_with_compucentric_rotation(self):
        stage = self.microscope.specimen.stage
        ...
        if self.test_helper.is_offline:
            self.skipTest("Compucentric rotation is not simulated in offline mode, skipping")

        x_origin = 0
        y_origin = 0.005
        x_compensated_45deg = 0.0035355
        y_compensated_45deg = 0.0035355
        x_compensated_90deg = 0.005
        y_compensated_90deg = 0.000

        print("Going to test origin...")
        p1 = StagePosition(x=x_origin, y=y_origin, r=0, t=0)
        ...
        p2 = StagePosition(r=math.radians(45))
        stage.absolute_move(p2, settings)
        self.__assert_stage_coordinates(stage.current_position, expected_x=x_compensated_45deg, expected_y=y_compensated_45deg, expected_r=math.radians(45))
        print("Success.")

        print("Performing compucentric rotation by 45 degrees using relative move...")
        settings = MoveSettings(rotate_compucentric=True)
        p3 = StagePosition(r=math.radians(45))
        stage.relative_move(p3, settings)
        self.__assert_stage_coordinates(stage.current_position, expected_x=x_compensated_90deg, expected_y=y_compensated_90deg, expected_r=math.radians(90))
        print("Success.")
        ...
        p4 = StagePosition(r=math.radians(45))
        stage.absolute_move(p4, settings)
        self.__assert_stage_coordinates(stage.current_position, expected_x=x_compensated_90deg, expected_y=y_compensated_90deg, expected_r=math.radians(45))
        print("Success.")

        print("Performing non-compucentric rotation by 45 degrees using relative move...")
        settings = MoveSettings(rotate_compucentric=False)
        p5 = StagePosition(r=-math.radians(45))
        stage.relative_move(p5, settings)
        self.__assert_stage_coordinates(stage.current_position, expected_x=x_compensated_90deg, expected_y=y_compensated_90deg, expected_r=math.radians(0))
        print("Success.")
    ...
    def test_move_stage_to_device(self):
        stage = self.microscope.specimen.stage

        # Happy flow is not tested here because it would take considerable time and would be in fact duplicate.
        # The functionality is covered by imaging_with_* tests in TestsImagingCameras.

        print("Moving to invalid devices must throw...")
        with self.assertRaisesRegex(ApplicationServerException, "Cannot move to"):
            stage.move_to_device(ImagingDevice.IR_CAMERA)
        with self.assertRaisesRegex(ApplicationServerException, "Cannot move to"):
            stage.move_to_device(ImagingDevice.CCD_CAMERA)
        with self.assertRaisesRegex(ApplicationServerException, "Cannot move to"):
            stage.move_to_device(ImagingDevice.VOLUMESCOPE_APPROACH_CAMERA)
        print("Success.")

        if self.test_helper.is_optical_microscope_installed:
            print("Moving to optical microscope with stage tilt < 52 must throw")
            position = StagePosition(t=0)
            stage.absolute_move(position)
            with self.assertRaisesRegex(ApplicationServerException, "Cannot move to"):
