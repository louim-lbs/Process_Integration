...
class TestsSpecimenTemperatureStage(unittest.TestCase):
    ...
    def __ensure_stage_is_installed(self, stage_type):
        is_installed = self.test_helper.is_temperature_stage_installed(stage_type)
        if not is_installed:
    ...
    def __connect_stage(self, stage_type):
        ...
        self.test_helper.connect_temperature_stage(stage_type)

        print(f"{stage_type} connected.")

    def __disconnect_stage(self, stage_type):
        print(f"Disconnecting {stage_type}...")
        self.test_helper.disconnect_temperature_stage(stage_type)
    ...
    def __calibrate_stage(self, stage_type):
        ...
        if stage_type == TemperatureStageType.MICRO_HEATER:
            configuration_key = self.test_helper.get_stage_configuration_key(stage_type) + ".Calibrated"
    ...
    def test_high_vacuum_heating_stage(self):
        ...
        print("Calling on high vacuum heating stage before heat shield is selected must throw...")
        with self.assertRaisesRegex(ApplicationServerException, "has not been selected yet"):
            temperature_stage.turn_on()
        with self.assertRaisesRegex(ApplicationServerException, "has not been selected yet"):
            temperature_stage.turn_off()
        with self.assertRaisesRegex(ApplicationServerException, "has not been selected yet"):
        ...
        print("Selecting heat shield...")
        self.microscope.specimen.temperature_stage.select_heat_shield(TemperatureStageHeatShieldType.TOP_DOWN)
