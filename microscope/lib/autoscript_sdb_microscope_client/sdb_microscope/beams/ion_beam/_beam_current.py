...
class BeamCurrent(object):
    ...
    def __init__(self, application_client):
        ...
        self.__id = "SdbMicroscope.Beams.IonBeam.BeamCurrent"
    ...
    def value(self) -> 'float':
        ...
        if call_response.result.data_type != DataType.DOUBLE:
            raise TypeError("Incompatible type: beam_current.value was expecting float, but server returned different object type: " + repr(call_response.result.data_type))
    ...
    def available_values(self) -> 'List[float]':
        ...
        if call_response.result.data_type != DataTypeDefinition(DataType.LIST_PRIMARY_ID, template_argument=DataType.DOUBLE):
            raise TypeError("Incompatible type: beam_current.available_values was expecting list, but server returned different object type: " + repr(call_response.result.data_type))
