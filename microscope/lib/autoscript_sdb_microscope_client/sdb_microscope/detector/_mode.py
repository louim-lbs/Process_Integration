...
class Mode(object):
    ...
    def __init__(self, application_client):
        ...
        self.__id = "SdbMicroscope.Detector.Mode"
    ...
    def available_values(self) -> 'List[str]':
        ...
        if call_response.result.data_type != DataTypeDefinition(DataType.LIST_PRIMARY_ID, template_argument=DataType.STRING):
            raise TypeError("Incompatible type: mode.available_values was expecting list, but server returned different object type: " + repr(call_response.result.data_type))
    ...
    def value(self) -> 'str':
        ...
        if call_response.result.data_type != DataType.STRING:
            raise TypeError("Incompatible type: mode.value was expecting str, but server returned different object type: " + repr(call_response.result.data_type))
