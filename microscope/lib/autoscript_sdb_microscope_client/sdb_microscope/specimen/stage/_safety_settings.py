...
class SafetySettings(object):    
    ...
    def locked_axes(self) -> 'List[str]':        
        ...
        if call_response.result.data_type != DataTypeDefinition(DataType.LIST_PRIMARY_ID, template_argument=DataType.STRING):
            raise TypeError("Incompatible type: safety_settings.locked_axes was expecting list, but server returned different object type: " + repr(call_response.result.data_type))
