...
class RampingSpeed(object):
    ...
    def __init__(self, application_client):
        ...
        self.__id = "SdbMicroscope.Specimen.TemperatureStage.RampingSpeed"
    ...
    def target_value(self) -> 'float':
        ...
        if call_response.result.data_type != DataType.DOUBLE:
            raise TypeError("Incompatible type: ramping_speed.target_value was expecting float, but server returned different object type: " + repr(call_response.result.data_type))
    ...
    def value(self) -> 'float':
        ...
        call_response = self.__application_client._perform_call(call_request)
        if call_response.result.data_type != DataType.DOUBLE:
            raise TypeError("Incompatible type: ramping_speed.value was expecting float, but server returned different object type: " + repr(call_response.result.data_type))
    ...
    def limits(self) -> 'Limits':
        ...
        if call_response.result.data_type != DataTypeDefinition(DataType.STRUCTURE_PRIMARY_ID, secondary_id="Limits"):
            raise TypeError("Incompatible type: ramping_speed.limits was expecting Limits, but server returned different object type: " + repr(call_response.result.data_type))
